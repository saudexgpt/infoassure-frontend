...
export default {
  ...
  methods: {
    ...
    saveTicket() {
      ...
      form.created_by = `${app.userData.name} (${app.userData.email})`
      const saveTicketsResource = new Resource('vdd/client-tickets/store')
      saveTicketsResource.store(form)
        .then(() => {
    ...
    fetchTicketAnalysis() {
      ...
      ticketsResource.list({ vendor_id: app.vendorId })
        .then(response => {
