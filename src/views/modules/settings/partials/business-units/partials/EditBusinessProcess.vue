<template>
  <div style="border: #333333 solid 3px; border-radius: 5px">
    <div class="justify-content-between align-items-center px-2 py-1">
      <el-button type="primary" :disabled="isImportantFieldsEmpty()" @click="update()">
        Update
      </el-button>
      <p></p>
  ...
  methods: {
    ...
    fetchBusinessUnits() {
      ...
      fetchBusinessUnitsResource
        .list({ client_id: this.businessProcess.client_id })
        .then((response) => {
          this.business_units = response.business_units
          this.business_units.forEach((unit) => {
            if (unit.id === this.businessProcess.business_unit_id) {
    ...
    update() {
      ...
      saveBusinessProcessResource
        ...
        .then(() => {
          ...
          this.$emit('updated')
          // this.$emit('update:is-edit-business-process-sidebar-active', false)
        })
        .catch((error) => {
          this.loading = false
