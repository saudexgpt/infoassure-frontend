<template>
  <el-card>
    <v-client-table :data="exceptions" v-loading="loading" :columns="columns" :options="options">
      <div v:slot="clause.name" v:slot-scope="{ row }">
        {{ `${row.clause.name} - ${row.clause.description}` }}
      </div>
...
export default {
  ...
  methods: {
    fetchExceptions() {
      ...
      this.loading = true
      const fetchQuestionsResource = new Resource('ndpa/exceptions/fetch')
    ...
    reverseException(row) {
      if (window.confirm('Are you sure you want to reverse this entry?')) {
        this.loading = true
        const destroyQuestionsResource = new Resource('ndpa/exceptions/reverse')
