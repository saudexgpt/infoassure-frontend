...
export default {
  ...
  methods: {
    ...
    makePayment(contract) {
      const app = this
      app.$confirm(`This will attest that you have actually made payment for contract ${contract.contract_no}. Continue?`, 'Warning', {
        confirmButtonText: 'Yes, Continue',
        cancelButtonText: 'Cancel',
        type: 'warning',
      }).then(() => {
        const fetchStaffResource = new Resource('vdd/contracts/make-payment')
        fetchStaffResource.update(contract.id)
          .then(response => {
            app.contract = response.contract
            this.$message({
              type: 'success',
