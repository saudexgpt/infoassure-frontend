      ...
      <div class="table-responsive">
        ...
        >
          ...
          <tbody v-if="!downloading">
            <template
              v-for="(report, index) in risks"
            >

              <tr :key="index">
                <td>
                  {{ (report.business_unit.teams.split(',').length > 0) ? report.business_unit.teams.split(',')[0] : '' }}
                </td>
                <td>
                  {{ (report.business_unit.teams.split(',').length > 1) ? report.business_unit.teams.split(',')[1] : '' }}
                </td>
                <td>
                  {{ (report.business_unit.teams.split(',').length > 2) ? report.business_unit.teams.split(',')[2] : '' }}
                </td>
                <td>
                  {{ report.business_process.generated_process_id }}
                </td>
                <td>{{ report.business_process.name }}</td>
                <td>{{ report.risk_unique_id }}</td>
                <td>
                  <textarea
                    v-model="report.description"
                    type="text"
                    class="form-control"
                    @blur="updateField($event, 'description', report)"
                  />
                </td>
                <td>
                  <input
                ...
                <td>
                  <!-- {{ report.business_process.objective }} -->
                  <!-- eslint-disable-next-line vue/no-v-html -->
                  <span v-html="report.business_process.objective" />
                </td>
          ...
          <tbody v-else>
            ...
            >
              ...
              <td>{{ report.control_no }}</td>
              <td>{{ report.control_location }}</td>
              <td>{{ report.control_description }}</td>
              <td>{{ report.control_owner }}</td>
              <td>{{ report.control_frequency }}</td>
              <td>{{ report.control_type }}</td>
              <td>{{ report.nature_of_control }}</td>
              <td>{{ report.application_used_for_control }}</td>
              <td>{{ report.compensating_control }}</td>
              <td>{{ report.test_procedures }}</td>
              <td>{{ report.sample_size }}</td>
              <td>{{ report.data_required }}</td>
              <td>{{ report.link_to_evidence }}</td>
              <td>{{ report.test_conclusion }}</td>
              <td>{{ report.gap_description }}</td>
              <td>{{ report.tod_improvement_opportunity }}</td>
              <td>{{ report.recommendation }}</td>
              <td>{{ report.responsibility }}</td>
              <td>{{ report.timeline }}</td>
              <td>{{ report.tod_gap_status }}</td>
            </tr>
          </tbody>
        </table>
        <br>
...
export default {
  ...
  methods: {
    ...
    updateField($event, field, assessment) {
      ...
      const updateResource = new Resource('risk-assessment/update-risk-fields')
