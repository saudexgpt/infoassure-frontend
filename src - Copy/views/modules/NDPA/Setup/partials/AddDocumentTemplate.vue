...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      formData.append('clause_id', app.section.clause_id)
      formData.append('section_id', app.section.id)
      const updatePhotoResource = new Resource('ndpa/clauses/upload-document-template')
      updatePhotoResource.store(formData)
        .then(() => {
          ...
          app.$emit('reload')
          app.$message('Template upload successful')
        })
        .catch(e => {
          app.load = false
